      ...
      {columns.length === 0 ? (
        ...
        <div style={{ background: '#0F1117', border: '1px solid #252836', borderRadius: 12, overflow: 'hidden' }}>
          ...
          {rows.length === 0 ? (
            ...
            <div key={row.id} style={{ display: 'grid', gridTemplateColumns: `200px ${columns.map(c => c.type === 'checkbox' ? '90px' : '1fr').join(' ')} 60px`, borderBottom: '1px solid #252836' }}
              className="table-row">
              <div style={TD}>
                ...
                <div key={c.id} style={{ ...TD, justifyContent: c.type === 'checkbox' ? 'center' : 'flex-start' }}>
                  {c.type === 'checkbox' ? (
                    <button
                      ...
                      style={{ width: 22, height: 22, borderRadius: '50%', border: `1.5px solid ${row[c.id] ? '#10B981' : '#2E3347'}`, background: row[c.id] ? '#10B981' : '#1C1F2A', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, padding: 0 }}>
